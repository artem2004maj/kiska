<!-- resources/js/Layouts/DirectorLayout.vue -->
<template>
    <div class="bg-gray-50 text-black/50 dark:bg-black dark:text-white/50 min-h-screen">
        <div class="relative flex min-h-screen flex-col selection:bg-[#8b5cf6] selection:text-white">
                ...
                <div class="flex flex-col lg:flex-row gap-3 lg:gap-6 py-3 lg:py-6">
                    ...
                    <div class="lg:hidden flex items-center justify-between mb-2 bg-white p-3 rounded-lg shadow-sm">
                        ...
                        <div class="flex items-center gap-2">
                            <span class="font-medium text-sm">{{ director?.employee_name?.split(' ')[0] || 'Директор' }}</span>
                            <div class="relative" ref="mobileProfileMenu">
                                <button @click="mobileProfileMenuOpen = !mobileProfileMenuOpen" 
                                        class="w-8 h-8 rounded-full bg-[#8b5cf6]/10 flex items-center justify-center overflow-hidden">
                                    <span class="text-xs font-medium text-[#8b5cf6]">
                                ...
                                <div v-if="mobileProfileMenuOpen" class="absolute right-0 mt-2 w-40 bg-white rounded-lg shadow-xl py-1 z-50 text-sm">
                                    <Link href="/director/profile" class="block px-3 py-2 hover:bg-gray-100">Профиль</Link>
                    ...
                    <aside class="hidden lg:block lg:w-56 xl:w-64 shrink-0">
                        <div class="bg-white dark:bg-zinc-900 rounded-lg p-4 shadow-[0px_14px_34px_0px_rgba(0,0,0,0.08)] ring-1 ring-white/[0.05] dark:ring-zinc-800 sticky top-10">
                            <div class="mb-6 flex justify-center">
                                <svg class="h-12 w-auto text-[#8b5cf6] lg:h-16" viewBox="0 0 62 65" fill="none">
                            ...
                            <nav class="space-y-1">
                                ...
                                <Link href="/director/services" 
                                      class="flex items-center gap-3 px-3 py-2 rounded-md transition text-sm lg:text-base"
                                      :class="isActive('/director/services')">
                                    <span class="text-lg lg:text-xl">💅</span>
                                    <span>Услуги</span>
                                </Link>
                                ...
                                <Link href="/director/supply-control" 
                                      class="flex items-center gap-3 px-3 py-2 rounded-md transition text-sm lg:text-base"
                                      :class="isActive('/director/supply-control')">
                                    <span class="text-lg lg:text-xl">🚚</span>
                                    <span>Поставки</span>
                                </Link>
                            </nav>
                        ...
                        <div v-if="mobileMenuOpen" class="lg:hidden fixed inset-0 z-40" @click="mobileMenuOpen = false">
                            ...
                            <div class="absolute left-0 top-0 h-full w-64 bg-white shadow-xl" @click.stop>
                                <div class="p-4 border-b">
                                    <p class="font-medium">{{ director?.employee_name || 'Директор' }}</p>
                                ...
                                <nav class="p-3 space-y-1">
                                    ...
                                    <Link href="/director/services" @click="mobileMenuOpen = false"
                                          class="flex items-center gap-3 px-3 py-2.5 rounded-md w-full text-sm"
                                          :class="isActive('/director/services')">
                                        <span class="text-lg">💅</span> Услуги
                                    </Link>
                                    
                                    <Link href="/director/staff" @click="mobileMenuOpen = false"
                                          class="flex items-center gap-3 px-3 py-2.5 rounded-md w-full text-sm"
                                          :class="isActive('/director/staff')">
                                        <span class="text-lg">👥</span> Персонал
                                    </Link>
                                    
                                    <Link href="/director/supply-control" @click="mobileMenuOpen = false"
                                          class="flex items-center gap-3 px-3 py-2.5 rounded-md w-full text-sm"
                                          :class="isActive('/director/supply-control')">
                                    ...
                                    <hr class="my-2">
                                    <Link href="/director/profile" @click="mobileMenuOpen = false"
                                          class="flex items-center gap-3 px-3 py-2.5 rounded-md w-full text-sm"
                        ...
                        <header class="hidden lg:flex items-center justify-between mb-4">
                            ...
                            <div class="relative" ref="profileMenu">
                                <button @click="showProfileMenu = !showProfileMenu" class="flex items-center gap-2 text-sm">
                                    <span class="hidden xl:inline">{{ director?.employee_name || 'Директор' }}</span>
                                ...
                                <div v-if="showProfileMenu" class="absolute right-0 mt-2 w-40 bg-white dark:bg-zinc-900 rounded-lg shadow-xl py-1 z-50 text-sm">
                                    <Link href="/director/profile" class="block px-3 py-2 hover:bg-gray-100 dark:hover:bg-zinc-800">Профиль</Link>
...
const isActive = (path) => {
    return currentRoute.value === path ? 'bg-[#8b5cf6] text-white' : 'text-gray-700 dark:text-white/70 hover:bg-[#8b5cf6]/10';
};
